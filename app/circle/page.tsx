...
export default function CirclePage() {
  ...
  // 정렬(최신/인기)과 필터(질문만 보기/답변 없는 질문만)를 하나의 상태로 묶어서
  // 항상 정확히 하나만 활성화되게 함 — 예전엔 3개 독립 state라 "인기순 + 질문만 보기"처럼
  // 동시에 여러 개가 켜진 것처럼 보이는 조합이 가능했음.
  const [filterMode, setFilterMode] = useState<"latest" | "popular" | "question" | "unanswered">("latest");
  const questionOnly = filterMode === "question" || filterMode === "unanswered";
  const unansweredQuestionOnly = filterMode === "unanswered";
  const sortMode = filterMode === "popular" ? "popular" : "latest";
  ...
  }, [posts, searchQuery, nicknames, filterMode, questionOnly, unansweredQuestionOnly, sortMode, commentCounts, reactionCounts]);
        ...
        <div className="mx-auto max-w-[1320px] xl:grid xl:grid-cols-[minmax(0,1fr)_1px_360px] xl:gap-x-10">
          <section className="min-w-0 px-5 sm:px-8 xl:pl-24" aria-label="Circle 게시글 피드">
            ...
            <div className="flex h-[72px] items-end gap-8 border-b border-[#f1efed]">
              <button type="button" onClick={() => setFilterMode("latest")} aria-pressed={filterMode === "latest"} className={`h-full border-b pb-4 pt-5 text-sm ${filterMode === "latest" ? "border-ink font-medium text-ink" : "border-transparent text-ink-soft hover:text-ink"}`}>
                최신
              </button>
              <button type="button" onClick={() => setFilterMode("popular")} aria-pressed={filterMode === "popular"} className={`h-full border-b pb-4 pt-5 text-sm ${filterMode === "popular" ? "border-ink font-medium text-ink" : "border-transparent text-ink-soft hover:text-ink"}`}>
                인기
              </button>
            </div>

            <div className="flex flex-wrap items-center gap-2 border-b border-[#f1efed] py-3">
              <button type="button" onClick={() => setFilterMode((mode) => (mode === "question" ? "latest" : "question"))} aria-pressed={filterMode === "question"} className={filterMode === "question" ? "rounded-md border border-accent bg-accent px-3 py-1.5 text-xs text-white" : "rounded-md border border-[#e6e4e1] px-3 py-1.5 text-xs text-ink-soft hover:border-ink hover:text-ink"}>
                질문만 보기
              </button>
              <button type="button" onClick={() => setFilterMode((mode) => (mode === "unanswered" ? "latest" : "unanswered"))} aria-pressed={filterMode === "unanswered"} className={filterMode === "unanswered" ? "rounded-md border border-accent bg-accent px-3 py-1.5 text-xs text-white" : "rounded-md border border-[#e6e4e1] px-3 py-1.5 text-xs text-ink-soft hover:border-ink hover:text-ink"}>
                답변 없는 질문만
              </button>
            </div>
